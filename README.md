...
`build/libs/discord-mcp-1.0.2.jar`(Shadow プラグインによる fat jar)が生成されます。
...
```json
{
  "mcpServers": {
    "discord": {
      "command": "java",
      "args": ["-jar", "D:\\Dev\\Discord-MCP\\build\\libs\\discord-mcp-1.0.2.jar"],
      "env": {
...
java -jar build\libs\discord-mcp-1.0.2.jar
```
